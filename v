...
with open("ventes.csv", "r") as fichier:
    ...
    for ligne in lecteur:
        ...
        # Chargement et affichage propre du fichier
import csv
...
with open("ventes.csv", "r") as fichier:
    ...
    for ligne in lecteur:
        print("{:<10} {:<10} {:<10} {:<10}".format(*ligne))
...
with open("ventes.csv", "r") as fichier:
    lecteur = csv.reader(fichier, delimiter=";")
    entete =next(lecteur)  #  ignore l'entête # Lecture de l'en-tête
 # Affichage de l'en-tête avec la colonne CA
    print("{:<10} {:<10} {:<10} {:<10} {:<10} {:<10} {:<10}".format(*entete, "CA_Brut","CA_Net", "TVA"))
    total = 0  #  variable pour le total

    max_ca = 0
    id_max = ""
      # 🔥 on stocke les lignes calculées
    resultats = []
    ...
    for ligne in lecteur:
        ...
        total += CA_Net  # 🔥 on ajoute au total
        # Affichage des données avec le CA
        print("{:<10} {:<10} {:<10} {:<10} {:<10} {:<10} {:<10}".format(*ligne, CA_Brut, CA_Net, TVA))

        ids.append(ligne[0])   # ID
        ca_nets.append(CA_Net) # CA Net
...
9. *(Bonus)* Graphique en barres avec Matplotlib `CA Net par produit`